...
from seraphsix.models.database import Member, Clan, ClanMember, GameMember, Game, ClanGame
...
class Database(object):
    ...
    async def get_member_by_naive_username(self, username, include_clan=True):
        ...
        if include_clan:
            member_db = await ClanMember.annotate(
                bungie_username_lo=Lower('member__bungie_username'), xbox_username_lo=Lower('member__xbox_username'),
                psn_username_lo=Lower('member__psn_username'), blizzard_username_lo=Lower('member__blizzard_username'),
                steam_username_lo=Lower('member__steam_username'), stadia_username_lo=Lower('member__stadia_username')
            ).get_or_none(
                Q(bungie_username_lo=username) | Q(xbox_username_lo=username) |
                Q(psn_username_lo=username) | Q(blizzard_username_lo=username) |
                Q(steam_username_lo=username) | Q(stadia_username_lo=username)
            ).prefetch_related('clan', 'member')
        else:
            member_db = await Member.annotate(
                bungie_username_lo=Lower('bungie_username'), xbox_username_lo=Lower('xbox_username'),
                psn_username_lo=Lower('psn_username'), blizzard_username_lo=Lower('blizzard_username'),
                steam_username_lo=Lower('steam_username'), stadia_username_lo=Lower('stadia_username')
            ).get_or_none(
                Q(bungie_username_lo=username) | Q(xbox_username_lo=username) |
                Q(psn_username_lo=username) | Q(blizzard_username_lo=username) |
                Q(steam_username_lo=username) | Q(stadia_username_lo=username)
    ...
    async def get_member_by_platform_username(self, username, platform_id):
        ...
        query = await Member.annotate(
            name_lo=Lower(username_field)
        ).get_or_none(name_lo=username).prefetch_related('clans')
        return query

    async def get_member_by_discord_id(self, discord_id, include_clan=True):
        if include_clan:
            query = ClanMember.get_or_none(member__discord_id=discord_id).prefetch_related('member', 'clan')
        else:
            query = Member.get_or_none(discord_id=discord_id)
        return await query

    async def get_clan_members(self, clan_ids):
        return await ClanMember.filter(clan__clan_id__in=clan_ids).prefetch_related('member', 'clan')

    async def get_clan_members_by_guild_id(self, guild_id):
        return await ClanMember.filter(clan__guild__guild_id=guild_id).prefetch_related('member', 'clan', 'clan__guild')
    ...
    async def get_clan_members_active(self, clan_db, **kwargs):
        ...
        return await ClanMember.filter(
            last_active__gt=timezone.now() - timedelta(**kwargs),
            clan=clan_db
    ...
    async def get_clan_members_inactive(self, clan_db, **kwargs):
        if not kwargs:
            kwargs = dict(days=30)
        return await ClanMember.filter(
            last_active__lt=timezone.now() - timedelta(**kwargs),
            clan=clan_db
        ).prefetch_related('member')
